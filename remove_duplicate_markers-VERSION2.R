...
# anaylzes, organizes, and tabulates the data into one main .csv file and several other 
# supporting .csv and .txt files. 
# 
...
#           this script, or even by yourself manually. The format of this file is simple:
...
# The directory path of the folder that the input files are in. This is also the 
# folder that the output files will be saved to.
path <- "/home/benrancourt/Downloads/r45-LepMAP2-final-copyForTesting/r45-inThreeStages"

# The name of the .cvs input file, minus the ".csv" extension.
csvFilenameMinusExtension <- "r45-60592-p0.01"

# The name of the input map file, minus the ".txt" extension. 
mapFilename <- "r45-60592-p0.01-map_js.txt"

# This is the expected first part of the OrderMarkers output filenames which are used
# as input to this script. The OrderMarkers output filenames are expected to follow this 
# convention:
...
# The marker id column in your input .csv file contains marker ids, which are composed of 
...
# id. Common separators are "-" and "_". All id's must use the same separator character to
# differentiate the gene portion from the marker portion. The script uses the separator 
# character to extract the gene id from the whole marker id string. Please indicate what 
# your input .csv file uses as the separator character.
separatorForGeneNameVsMarkerId <- "-"

# Some of the data in your original input .csv will likely be missing, in which case it 
# might be represented as NA or "-" or something like that to indicate that it is missing.
...
# other TODO:
# x- parametrize separatorForGeneNameVsMarkerId
# x- create Input Parameters section
# x- make path more robust
# x- explicitly define the name structure of the OrderMarkers .SA.txt files? 
# x- parametrize the file extensions
# - parametrize the Pinus Consensus LG id matching
# - generate .linkage (actually, the LepMAP2pipeline.sh should do that, in addition to indicating whether
# to run SeparateChromosomes and JoinSingles again or go straight to OrderMarkers).
