...
path="."
inputCsvFile="LP36_pipe1_full-sampleLines.csv"

outputCsvFileNameBase="yLP36_pipe1-sampleLines-filtered" 
...
cat $path/$inputCsvFile | awk -F ',' -v header="$headerLine" -v indelStatus="$indelStatus" -v naVal="$naValue" -v refCol=$refColName -v allowMissing=$allowXmissingRSplusSSdata -v mafCut=$mafCutoff -v allGenotypes="$genotypeColNames" -v rsGenotypes="$RSgenotypeColNames" -v ssGenotypes="$SSgenotypeColNames" -v outFileBase="$path/$outputCsvFileNameBase" '
...
function generateGenoStats(allGenoIndexes, naValue, isIndel, statsArr,
                           ####  local-scope variables below here
                           totalA, totalC, totalG, totalT, totalEmpty, i, maf, indel1, indel2, other_indels, indel1_val, indel2_val, indelsDict, indelsStats, tmpArr, key) {
...
BEGIN { 
  ...
  if (indelStatus == 2 || indelStatus == 3) {
    ...
    indelOutFileMin1 = outFileBase"-indels-minimal1.csv"
    indelOutFileMin2 = outFileBase"-indels-minimal2.csv"
